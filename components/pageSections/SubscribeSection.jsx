...
const SubscribeSection = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (valid) {
      try {
        const response = await strapiPost("contact-requests", {
          email,
          type: "subscribe",
        });
          ...
          <div className="mx-auto flex flex-col lg:flex-row items-center justify-between 2xl:gap-[230px] xl:gap-[140px] lg:gap-[65px] sm:gap-6 gap-4">
            {succsessMessage ? (
              <div className="w-full flex lg:flex-row flex-col justify-center items-center min-h-[120px] sm:gap-[18px] gap-3">
                <svg
                  className="1xl:w-12 1xl:h-12 sm:w-11 sm:h-11 w-10 h-10"
                  width="50"
                  height="50"
                  viewBox="0 0 50 50"
                  fill="none"
                ...
                <h2 className="lg:text-start text-center text-white 2xl:text-3xl 1xl:text-[32px] xl:text-[30px] sm:text-[27px] text-[22px] font-medium xl:leading-[45px] lg:leading-10 leading-8">
                  {succsessMessage}
                </h2>
              </div>
            ) : (
              <>
                <div className="sm:max-w-[626px] w-full lg:text-start sm:text-center text-start">
                  {title && (
                    <h2 className="text-white 2xl:mb-[18px] xl:mb-3 mb-2">
                      {title}
                    </h2>
                  )}
                  {label && (
                  ...
                  <div className="w-full 2xl:max-w-[600px] sm:max-w-[530px]">
                    <form onSubmit={handleSubmit}>
                      <div>
                        ...
                        {emailError && (
                          <p className="text-red-500 text-sm mt-1 bg-white py-[2px] px-[11px] rounded-[5px] w-fit tracking-[0.5px]">
                            {emailError}
                          </p>
                        )}
                      </div>
                      ...
                      {checkboxError && (
                        <p className="text-red-500 text-sm mt-1 bg-white py-[2px] px-[11px] rounded-[5px] w-fit">
                          {checkboxError}
                        </p>
                      )}
                    </form>
