...
export default function SideCart() {
    ...
    const { isCartOpen, removeFromCart, closeCart, cartItems = [], totalPrice = 0 } = useCart() || {}
        ...
        <>
            ...
            >
                <div className="flex items-center justify-between p-4 bg-blue-300">
                    <h5 className="sm:text-[20px] text-lg font-medium">Cart ({cartItems?.length || 0})</h5>
                ...
                <div className="overflow-y-auto sm:max-h-[calc(100vh-235px)] max-h-[calc(100vh-210px)] sm:p-5 p-3 space-y-[25px] h-full">
                    {Array.isArray(cartItems) && cartItems?.length > 0 ? (
                        cartItems?.map((item) => {
                            ...
                            return (
                                <div key={item?.id || Math.random()} className="flex items-start sm:gap-[18px] gap-3">
                                    <img
                                        src={imageUrl}
                                        alt={title}
                                    ...
                                    <button onClick={() => removeProductFromCart(item)} className="text-gray-400 hover:text-red-500 sm:ml-[10px] mt-1">
                ...
                <div className="sm:py-[18px] sm:px-5 p-3 shadow-gray">
                    ...
                    <button onClick={() => router.push(`/cart`)} className="w-full btn btn-outline-primary transition mb-3">View Cart</button>
                    <button onClick={() => router.push(`/checkout`)} className="w-full btn btn-primary transition">Checkout</button>
                </div>
            </div>
        </>
    )
