...
export default function FormInput({
  ...
  const labelText = data?.label ? `${data.label}${getRules(data.rules)}` : "";
  const inputId = data?.id || data?.name || undefined;
  ...
  return (
    <div className="w-full">
      ...
      xl:py-[23px] sm:py-[21px] py-[20px]
      !font-normal border outline-none flex
      ${data?.classNames?.inputWrapper || ""}
    `,
          innerWrapper: `${data?.classNames?.innerWrapper || ""}`,
        ...
        startContent={
          data?.startContent ? (
            ...
            <span className="bg-blue-300 text-gray-300 p2 whitespace-nowrap rounded-[5px] mr-2 flex items-center p-3">
              {data.prefix}
            </span>
          ) : null
        }
