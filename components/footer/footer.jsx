...
const fallbackMenuItems = {
    "Products and Partners": [
        ...
        { id: 2, label: "WooCommerce Themes", slug: "/woocommerce-themes", tag: null },
    ],
    Topics: [
        { id: 1, label: "Business & Services", slug: "/business-services", tag: null },
    ...
    Earn: [{ id: 1, label: "Become an author", slug: "/become-author", tag: null }],
...
export function Footer({ footerMenu, footerSettings }) {
    ...
    const organizedMenu = useMemo(() => {
        ...
        if (!Array.isArray(footerMenu) || footerMenu.length === 0) {
            ...
            return [
                ...
                { id: 2, label: "Topics", positioin: null, sub_menu: fallbackMenuItems["Topics"] },
                { id: 3, label: "Company", positioin: null, sub_menu: fallbackMenuItems["Company"] },
                { id: 4, label: "Earn", positioin: null, sub_menu: fallbackMenuItems["Earn"] },
                { id: 5, label: "Support", positioin: null, sub_menu: fallbackMenuItems["Support"] },
            ]
        }
    ...
    const productsAndPartners = organizedMenu.find((item) => item.label === "Products and Partners")
    ...
    const socialLinks = useMemo(() => {
        ...
        return footerSettings.social_media
            ...
            .map((social) => {
                ...
                const label =
                    social.label ||
                    Object.keys(socialIconMap).find((key) => social.link.toLowerCase().includes(key)) ||
                    "Social Media"
    ...
    // Get the logo
    const logo = footerSettings?.logo || { url: "/placeholder.svg?height=40&width=240", width: 240, height: 40 }
                ...
                <ul className="space-y-2">
                    {subMenu.map((subItem, index) => (
                        <li key={subItem.id || index}>
                            <Link href={subItem.slug || "#"} className="text-[#bbbbbb] hover:text-white transition-colors">
                                {subItem.label}
                            </Link>
                        </li>
                    ))}
    ...
    return (
        <footer className="bg-[#1a2127] text-white">
            {/* Top section with logo and buttons */}
            <div className="container mx-auto px-4 py-8 flex flex-col md:flex-row justify-between items-center border-b border-[#27313b]">
                ...
                <div className="flex space-x-4">
                    {buttons.map((btn) => (
                        <Button
                            ...
                            className={
                                btn.id === 1
                                    ? "text-white border-white hover:bg-[#27313b]"
                                    : "bg-[#0156d5] hover:bg-[#0156d5]/90 text-white"
                            }
                            asChild
                        >
                            <Link href={btn.link || "#"}>
                                {btn.image ? (
                                    <Image src={btn.image || "/placeholder.svg"} alt="" width={20} height={20} className="mr-2" />
                                ) : btn.id === 2 ? (
                                    socialIcons.clock
                                ) : null}
                                <span className={btn.image ? "ml-2" : ""}>{btn.label}</span>
                            </Link>
                        </Button>
                    ))}
                </div>
            ...
            <div className="container mx-auto px-4 py-6 border-t border-[#27313b] flex flex-col md:flex-row justify-between items-center">
                <p className="text-sm text-[#bbbbbb] mb-4 md:mb-0">{copyrightText}</p>
                <div className="flex space-x-4">
                    {socialLinks.map((social, index) => (
                        <Link
                            ...
                            className="bg-[#27313b] p-2 rounded-md hover:bg-[#0156d5] transition-colors"
                        >
                            {social.icon ? (
                                <Image src={social.icon || "/placeholder.svg"} alt="" width={20} height={20} />
                            ) : (
                                socialIcons[socialIconMap[social.label.toLowerCase()] || "facebook"]
                            )}
                        </Link>
                    ))}
                </div>
