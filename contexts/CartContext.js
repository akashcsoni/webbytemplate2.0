...
import { trackAddToCart, trackRemoveFromCart } from "@/lib/utils/trackUser";
...
export function CartProvider({ children }) {
  ...
  const removeFromCart = async (productId) => {
    ...
    // ------------------------------
  // 🍪 AUTO SET CART FLAG COOKIE (for middleware)
  // ------------------------------
  useEffect(() => {
    if (cartItems && cartItems.length > 0) {
      Cookies.set("cartHasItems", "true", { path: "/", expires: 1 }); // expires in 1 day
    } else {
      Cookies.set("cartHasItems", "false", { path: "/", expires: 1 });
    }
  }, [cartItems]);
