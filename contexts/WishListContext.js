...
export function WishListProvider({ children }) {
    ...
    const initializeWishlist = async () => {
        ...
        try {
            ...
            if (isLoggedIn) {
                ...
                try {
                    const res = await strapiGet(`wishlists/${existingWishlistId}`, { token })
                    if (res?.data) {
                        const { id, totalPrice, products } = res.data
                        setWishListId(id)
                        setTotalPrice(totalPrice || 0)
    ...
    const addToWishlist = async (product) => {
        try {
            ...
            const existingIndex = normalizedWishlist.findIndex(
                item => item.product === (product.product?.documentId || product.product?.id || product.product)
            );
            ...
            // Update the wishlist
            const res = await strapiPut(`wishlists/${currentId}`, wishlistData, themeConfig.TOKEN);
    ...
    const removeFromWishlist = async (productId) => {
        ...
        if (productId) {
            ...
            try {
                const res = await strapiPost(`/wishlist-item-remove/${wishlistId}`, productData, themeConfig.TOKEN)
